...
class ProviderServices { 
	...
	public function checkRequest($request) {
		...
		try{
			...
			if(!empty($IncomingRequests)){
				...
				$this->provider_cancel($IncomingRequests, $settings);
			}
	...
	public function provider_cancel($IncomingRequests, $settings) {
			...
			try {
				if($request->admin_service == "DELIVERY" ) {
					...
					try{
						...
						$newRequest = \App\Models\Delivery\DeliveryRequest::with('deliveries')->where('id', $request->id)->first();
			...
			//try {
				...
				if($request->admin_service == "ORDER" ) {
					...
					if($request->admin_service == "TRANSPORT") { 
						...
						if($config->broadcast_request != 1 || $ride_request->type == 'AIRPORT')
				        {
	...
	public function assignNextProvider($request_id, $admin_service) 
	{
		...
		try {
			...
			if($nextRequestFilter != null) {
				...
				\App\Models\Delivery\Delivery::where('delivery_request_id', $newRequest->id)->update(['status' => 'CANCELLED']);
