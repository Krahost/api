...
class AdminController extends Controller
{
	...
	{    
		...
		if($setting != null) {
			...
			foreach($request->only(['store_search_radius', 'store_provider_select_timeout', 'store_manual_request', 'order_otp','store_booking_prefix','max_items_in_order','product_timeline']) as $key => $req) {
			...
			if(count($request->adminservice) > 0 &&$request->has('adminservice')){
	...
	public function dashboarddata($id)
	{
	  try{
		  ...
		  $data['providerdata']=Provider::where('country_id', $id)->when(Auth::user()->type == 'FLEET', function ($q) {return $q->where('admin_id', Auth::user()->id);})->where('company_id',\Auth::user()->company_id)->count();
