...
class HomeController extends Controller
{
	...
	public function update_profile(Request $request)
	{
		...
		try{
			...
			if($request->has('iso2')) $provider->iso2 = $request->iso2;
	...
	public function adminservices(Request $request)
	{
		...
		foreach ($adminservices_list as $key => $adminservice) {
			$type= $adminservice->admin_service;
			$category_ids = '';
			$sub_category_ids ='';
			if($adminservice->admin_service == 'SERVICE') {
				$category_ids = ProviderService::where('admin_service',$adminservice->admin_service)->where('provider_id',Auth::guard('provider')->user()->id)->get()->pluck('category_id');
			       $sub_category_ids = ProviderService::where('admin_service',$adminservice->admin_service)->where('provider_id',Auth::guard('provider')->user()->id)->get()->pluck('sub_category_id');

			}
			$data = new \stdClass;
			$documents = Document::with(['provider_document','serviceCategory','servicesubCategory'])->where('company_id', Auth::guard('provider')->user()->company_id)->where('type',$adminservice->admin_service)->where('status',1)->when($adminservice->admin_service == 'SERVICE', function ($q) use( $category_ids,$sub_category_ids,$type) {
				if(!empty($category_ids)) {
					$q->whereIn('service_category_id',$category_ids);
					$q->orwhereIn('service_subcategory_id',$sub_category_ids);
				}

				$q->orwhereNull('service_category_id')->where('type',$type);
			})->get();
	...
	public function addvechile(Request $request)
	{
		...
		try{
			if($request->admin_service != 'SERVICE'){
				...
				$providervehicle->status="ASSESSING";
	...
	public function services_status(Request $request,$id)
	{
		if(!empty($id))
		{
			$provider_service = ProviderService::where('admin_service',$request->admin_service)->where('provider_vehicle_id',$id)->first();
			
			if($provider_service->status == "ASSESSING") {

		     	return Helper::getResponse(['status' => 404, 'message' => "Waiting For Admin Approval"]);
		    }
	...
	public function editvechile(Request $request)
	{
		...
		try{
			...
			if($request->admin_service=='SERVICE'){
				ProviderService::where('admin_service','SERVICE')->where('status','!=',"ASSESSING")->where('sub_category_id',$request->sub_category_id)->where('category_id',$request->category_id)->where('provider_id',Auth::guard('provider')->user()->id)->update(['status'=>"INACTIVE"]);
				if($request->has("service")){
					foreach($request->service  as $key =>$val){

						$existingservice = ProviderService::where('admin_service','SERVICE')->where('service_id',$val['service_id'])->where('sub_category_id',$request->sub_category_id)->where('category_id',$request->category_id)->where('provider_id',Auth::guard('provider')->user()->id)->first();

						if(!empty($existingservice) && $existingservice->status == "ASSESSING")
							$status = "ASSESSING";
						else
							$status = $existingservice ? "ACTIVE" : "ASSESSING";

		                $base_fare=isset($val['base_fare'])? $val['base_fare'] : 0.00;

		                $per_miles= isset($val['per_miles']) ? $val['per_miles']: 0.00;    
		               
		                $per_mins=isset($val['per_mins']) ? $val['per_mins'] : 0.00;
			    ...
			    $provider_service->status="ASSESSING";
		...
		try{
			...
			if($request->has('limit')) {
						...
						$query->where(function ($q) {
	                		return $q->where('user_id', Auth::guard('provider')->user()->id)->orwhereNull('user_id');
	            		});
            			$query->update(['is_viewed' => 1]);
						$notifications = $query->where('status','active')->whereDate('expiry_date','>=',Carbon::today())->orderby('id','desc')->take($request->limit)->offset($request->offset)->get();
			}else{
					$query = Notifications::where('company_id', Auth::guard('provider')->user()->company_id)->whereIn('notify_type',['provider','all']);
				...
				$notifications =$query->where('status','active')->whereDate('expiry_date','>=',Carbon::today())->orderby('id','desc')->paginate(10);  
			}
    ...
    public function wallet_transfer(Request $request){
   	...
   	{    
        ...
        try{    
        	...
        	ProviderService::where('admin_service','SERVICE')->where('status','!=',"ASSESSING")->where('provider_id',Auth::guard('provider')->user()->id)->update(['status'=>"INACTIVE"]);

            foreach($request->service_id  as $key =>$val){

				$existingservice = ProviderService::where('admin_service','SERVICE')->where('service_id',$val)->where('sub_category_id',$request->sub_category_id[$key])->where('category_id',$request->category_id[$key])->where('provider_id',Auth::guard('provider')->user()->id)->first();
				if(!empty($existingservice) && $existingservice->status == "ASSESSING")
					$status = "ASSESSING";
				else
					$status = $existingservice ? "ACTIVE" : "ASSESSING";

                $base_fare=isset($request->base_fare[$key])? $request->base_fare[$key] :   0.00;

                $per_miles= isset($request->per_miles[$key]) ? $request->per_miles[$key] : 0.00;    
               
                $per_mins=isset($request->per_mins[$key]) ? $request->per_mins[$key] : 0.00;
