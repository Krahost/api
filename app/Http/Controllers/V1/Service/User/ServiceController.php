...
class ServiceController extends Controller
{
	...
	public function promocode(Request $request)
	{
		...
		$promocode = Promocode::where('company_id', Auth::guard('user')->user()->company_id)->where('service', 'SERVICE');
        ...
        $promocode->Where(function ($query) {
            ...
            $query->orwhere('startdate','<=',Carbon::parse(Auth::guard('user')->user()->created_at)->format("Y-m-d H:i"));
        });
        $promocode->Where(function ($query) {
		            $query->whereNull('user_id');
		            $query->orWhere('user_id',Auth::guard('user')->user()->id);    
		        });
        $promocode->whereRaw("user_limit > (select ifnull((SELECT COUNT(*) from promocode_usages where promocode_id = promocodes.id and user_id = ".Auth::guard('user')->user()->id." group by promocode_id), 0) )");
	...
	public function requestStatementHistory(Request $request)
	{
		try {
			...
			if($request->has('search_text') && $request->search_text != null) {
				$serviceRequests->ProviderhistorySearch($request->search_text);
			}
			...
			foreach($dataval as $service){
				...
				$earnings = ServiceRequestPayment::select('total','commision')->where('service_request_id',$serviceid)->where('company_id',  Auth::user()->company_id)->first();
				if($earnings != null){
					$service->earnings = $earnings->total;
					$service->commision = $earnings->commision;
